...
String? _e(String n) => Platform.environment[n]?.isNotEmpty == true
    ? Platform.environment[n]
    : null;
...
void main() {
  setUpAll(() {
    if (!_ok) return;
    _c = SumUpClient.withApiKey(_key!);
  });
  ...
  void testApi(String name, Future<void> Function() fn, {bool skip = false}) {
    test(name, () async {
      ...
      try {
        ...
        print('  ⚠️  $name — HTTP ${e.response?.statusCode}');
      }
    });
  }
  ...
  group('Merchant', () {
    testApi('me/merchant-profile', () async {
      final r =
          await _c!.dio.get<Map<dynamic, dynamic>>('/v0.1/me/merchant-profile');
  ...
  group('Checkouts', () {
    testApi('createCheckout', () async {
      ...
      final r =
          await _c!.dio.post<Map<dynamic, dynamic>>('/v0.1/checkouts', data: {
  ...
  group('Transactions', () {
    testApi('listTransactions has items', () async {
      final r = await _c!.dio.get<Map<dynamic, dynamic>>(
          '/v2.1/merchants/$_mc/transactions/history');
      final data = r.data;
  ...
  group('Readers', () {
    testApi('listReaders has paired reader', () async {
      final r = await _c!.dio
          .get<Map<dynamic, dynamic>>('/v0.1/merchants/$_mc/readers');
  ...
  group('Members', () {
    testApi('listMembers has data', () async {
      final r = await _c!.dio
          .get<Map<dynamic, dynamic>>('/v0.1/merchants/$_mc/members');
      final items = r.data!['items'] as List;
  ...
  group('Roles', () {
    testApi('listRoles has Admin', () async {
      final r = await _c!.dio
          .get<Map<dynamic, dynamic>>('/v0.1/merchants/$_mc/roles');
      final items = r.data!['items'] as List;
      expect(items.isNotEmpty, isTrue);
      final admin = items
          .cast<Map<dynamic, dynamic>>()
          .firstWhere((r) => r['name'] == 'Admin');
  ...
  group('Customers', () {
    testApi('createCustomer', () async {
      final r =
          await _c!.dio.post<Map<dynamic, dynamic>>('/v0.1/customers', data: {
        'customer_id': 'dart-test-cust-001',
        'personal_details': {
          'first_name': 'James',
          'last_name': 'Bond',
          'email': 'purplesoft@sumup.com'
        },
      });
    ...
    testApi('getCustomer', () async {
      final r = await _c!.dio
          .get<Map<dynamic, dynamic>>('/v0.1/customers/dart-test-cust-001');
      final d = r.data!;
  ...
  group('Transaction detail', () {
    testApi('getTransaction by id', () async {
      ...
      final hist = await _c!.dio.get<Map<dynamic, dynamic>>(
          '/v2.1/merchants/$_mc/transactions/history');
      final txId = (hist.data!['items'] as List).first['id'] as String;
      // Then fetch it by ID
      final r = await _c!.dio.get<Map<dynamic, dynamic>>(
          '/v2.1/merchants/$_mc/transactions',
